...
$(BUILD_DIR)/copperos.iso: $(BUILD_DIR)/copperos.img
	...
	# Create a temporary ISO root containing the raw disk image so we can
	# produce an El Torito hybrid ISO with hard-disk emulation.
	rm -rf $(BUILD_DIR)/iso_root
	mkdir -p $(BUILD_DIR)/iso_root
	cp $(BUILD_DIR)/copperos.img $(BUILD_DIR)/iso_root/
	# macOS: use hdiutil; Linux/other: try genisoimage or xorriso
	if [ "$(UNAME_S)" = "Darwin" ]; then \
		...
		if command -v genisoimage >/dev/null 2>&1; then \
			genisoimage -o $(BUILD_DIR)/copperos.iso -V COPPEROS -J -r -eltorito-boot copperos.img -hard-disk-boot $(BUILD_DIR)/iso_root; \
		elif command -v xorriso >/dev/null 2>&1; then \
			xorriso -as mkisofs -o $(BUILD_DIR)/copperos.iso -V COPPEROS -J -r -eltorito-boot copperos.img -hard-disk-boot $(BUILD_DIR)/iso_root; \
		else \
			echo "genisoimage or xorriso required to build ISO on non-macOS systems"; exit 1; \
		fi; \
	fi
	rm -rf $(BUILD_DIR)/iso_root
